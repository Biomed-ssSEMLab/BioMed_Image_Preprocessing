...
class Section(object):
    ...
    def __init__(self, mfovs_dict, **kwargs):
        ...
        self._min_mfov_idx = np.array(list(mfovs_dict.keys())).min()
        self._max_mfov_idx = np.array(list(mfovs_dict.keys())).max()
    ...
    def create_from_tilespec(cls, tilespec, **kwargs):
        ...
        # add the last sec from the upper wafer
        wafer_num1, sec_num1 = kwargs["wafer_section"]
        if wafer_num1 == 2:
            layer = layer + 557
        elif wafer_num1 == 3:
            layer = layer + 1022
        ...
        #print("all_mfovs type:", type(all_mfovs))
    ...
    def _read_lines(cls, input_file):
        print("detect osfs: section.py")
        if input_file.startswith('/') or input_file.startswith('osfs://'):
            return Section._read_lines_using_cat(input_file.replace('osfs://', ''))
    ...
    @classmethod
    def _parse_coordinates_file(cls, input_file, limit_mFov=False, min_mFov_idx=1, max_mFov_idx=1):
...
#        for line in sorted_lines:
        for line in Section._read_lines(input_file):
            ...
            if limit_mFov and int(img_fname.split('/')[0]) not in range(min_mFov_idx, max_mFov_idx + 1):
                continue

            if "thumbnail" not in img_fname.split('/')[-1]:
                img_sec_mfov_beam = '_'.join(img_fname.split('/')[-1].split('_')[:3]) # 对于正常tile图像，命名像：001_000001_001_2022-07-25T1047078726257.bmp
            else:
                img_sec_mfov_beam = '_'.join(img_fname.split('/')[-1].split('_')[:4]) # 对于正常tile缩略图像，命名像：thumbnail_001_000001_001_2022-07-25T1047078726257.bmp
            ...
            if img_sec_mfov_beam not in images_dict.keys():
                ...
                if curr_img_date > prev_img_date:
                    ...
                    # images_dict[img_sec_mfov_beam] = img_fname # hqjin 之前代码应该写错了，images_dict[img_sec_mfov_beam]记录的是索引，直接注释掉就行
                    cur_x = float(line_data[1])
                    cur_y = float(line_data[2])
    ...
    @classmethod
    def create_from_full_image_coordinates(cls, full_image_coordinates_fname, layer, limit_mFov=False, min_mFov_idx=1, max_mFov_idx=1, \
        cur_fs = fs.open_fs("/"), tile_size=None, relevant_mfovs=None, **kwargs):
        """
        ...
        images, x_locs, y_locs = Section._parse_coordinates_file(full_image_coordinates_fname_fs, limit_mFov, min_mFov_idx, max_mFov_idx)
        assert(len(images) > 0)
...
#         for line in sorted_lines:
    ...
    def canonical_section_name(self):
        ...
        if self._canonical_section_name is None:
            ...
            # self._canonical_section_name = '{}_W{}_Sec{}'.format(str(self._layer).zfill(4), str(wafer).zfill(2), str(section).zfill(3))  # 源代码生成的文件名很累赘
            
            # add the last sec from the upper wafer
            wafer_num1, sec_num1 = self._wafer_section
            if sec_num1 > 557:
                layer1 = sec_num1
            else:
                layer1 = self._layer

            if self._min_mfov_idx == self._max_mfov_idx:
                self._canonical_section_name = 'S{}M{}'.format(str(layer1), str(self._min_mfov_idx))
            else:
                self._canonical_section_name = 'S{}M{}-{}'.format(str(layer1), str(self._min_mfov_idx), str(self._max_mfov_idx))
    ...
    def canonical_section_name_no_layer(self):
        ...
        """
        # return self.canonical_section_name[5:]  # 源代码生成的文件名很累赘
        return self.canonical_section_name
