...
class Mesh(object):
    ...
    def internal_structural_mesh(self):
        ...
        edge_indices = np.vstack((simplices[:, :2],
                                  simplices[:, 1:],
                                  simplices[:, [0, 2]]))
        edge_indices = np.vstack({tuple(sorted(tuple(row))) for row in edge_indices}).astype(np.uint32)  # 这种写法可以去除重复的边
    ...
    def remove_unneeded_points(self, pts1, pts2):
        ...
        p1 = pts1.copy()
        p1[p1 < 0] = 0.01
        simplex_indices = self.triangulation.find_simplex(p1)  # 该函数返回点在哪个三角面片中的索引，返回值-1表示点不在所有三角面片中
...
class ElasticMeshOptimizer(object):
    ...
    def _save_checkpoint_data(self, block_num, relevant_sec_idxs, meshes, links, norm_weights, structural_meshes):
        ...
        norm_weights = {(sec1_idx, sec2_idx): v for (sec1_idx, sec2_idx), v in norm_weights.items() \
                        if sec1_idx in relevant_sec_idxs or sec2_idx in relevant_sec_idxs}
    ...
    def _load_most_recent_checkpoint(self):
        block_num = 0
        meshes, links, norm_weights, structural_meshes = {}, {}, {}, {}
        ...
        if cp_fname is not None:
            ...
            '''
            print("there is no checkpoint block data")
            cp_fname2 = '/root/Data/RB98_retake/wafer1/result/alignment/checkpoint_block_000.0.pkl'
            with open(cp_fname2, 'rb') as in_file:
                block_num, meshes, links, norm_weights, structural_meshes = pickle.load(in_file)
            '''
        ...
        for active_sec_idx in range(pre_alignment_block_lo, block_hi):
            ...
            for neighbor_sec_idx in layout['neighbors'][active_sec_idx]:
                if neighbor_sec_idx < active_sec_idx:
                    ...
                    for (sec1_idx, sec2_idx), ((idx1, w1), (idx2, w2)) in [((active_sec_idx, neighbor_sec_idx), links[active_sec_idx, neighbor_sec_idx]), \
                        ((neighbor_sec_idx, active_sec_idx), links[neighbor_sec_idx, active_sec_idx])]:
                        ...
                        logger.report_event("Matches # (sections {}->{}): {}.".format(layout['sections'][sec1_idx].canonical_section_name, \
                                            layout['sections'][sec2_idx].canonical_section_name, pts1.shape[0]), log_level=logging.INFO)
        ...
        for iter in range(self._max_iterations):
            ...
            for sec_idx in range(block_lo, block_hi):
                cost += mesh_derivs_elastic.internal_grad(self._compute_mesh_area, meshes[sec_idx].pts, gradients[sec_idx],
                                                  *((structural_meshes[sec_idx]) + (self._intra_slice_weight, self._intra_slice_winsor)))
...
#                         #plot_points(meshes[active_ts].pts, os.path.join(DEBUG_DIR, "post_iter{}_{}.png".format(str(iter).zfill(5), os.path.basename(active_ts).replace(' ', '_'))))
            ...
            if stepsize < self._min_stepsize:
                break
        logger.report_event("region [{}, {}], last MO: {}\n".format(block_lo, block_hi, \
            mean_offsets(meshes, links, block_hi, plot=False, from_sec_idx=block_lo)), log_level=logging.INFO)
...
#                 os.makedirs(DEBUG_DIR)
        ...
        meshes, links, norm_weights = {}, {}, {}

        '''        
        #load last meshes
        filename = '/root/Data/RB98_retake/wafer1/result/alignment/checkpoint_block_548.0.pkl'
        last_mesh_index = 548
        with open(filename, 'rb') as in_file:
            last_nums, last_meshes, last_links, last_norm_weights, last_structural_meshes = pickle.load(in_file)
            last_mesh = last_meshes[last_mesh_index]
            last_structural_mesh = last_structural_meshes[last_mesh_index] 
        '''
        ...
        if self._checkpoints_dir is not None:
            ...
            print("the checkpoints dir load is: ", self._checkpoints_dir)
...
#             #plot_points(meshes[active_ts].pts, os.path.join(DEBUG_DIR, "pre_affine_{}.png".format(os.path.basename(active_ts).replace(' ', '_'))))
        ...
        for block_lo in (range(int(cp_block_num) * self._block_step, max(1, len(layout['sections'])), self._block_step)):

            block_hi = min(block_lo + self._block_size, len(layout['sections']))

            '''
            #debug the block_lo
            print("the block_lo:", block_lo)
            time.sleep(5)
            print("the block_hi:", block_hi)
            time.sleep(5)
            '''
            ...
            #print("last_block, block_hi, len(sections)", last_block, block_hi, len(layout['sections']))
            #time.sleep(5)

            # Load needed meshes
            meshes, links, norm_weights, structural_meshes = self._create_meshes_from_layout(layout, matches, block_lo, block_hi, meshes, \
                links, norm_weights, structural_meshes)
           
            '''
            if block_lo == 0:
                #change meshes last
                print("change last mesh")
                meshes[0] = last_mesh
                structural_meshes[0] = last_structural_mesh 
            '''
            ...
            logger.report_event("region [{}, {}], After preopt MO: {}".format(block_lo, block_hi, mean_offsets(meshes, links, block_hi, \
                plot=False, from_sec_idx=block_lo)), log_level=logging.INFO)
...
#                     # plot_points(meshes[active_ts].pts, os.path.join(DEBUG_DIR, "post_affine_{}.png".format(os.path.basename(active_ts).replace(' ', '_'))))
            ...
            for active_sec_idx in exported_sec_idxs:
                ...
                res = pool.apply_async(export_mesh_func, (layout['sections'][active_sec_idx], meshes[active_sec_idx].orig_pts, \
                                       meshes[active_sec_idx].pts, self._mesh_spacing))
                pool_results.append(res)
            for res in pool_results:
                res.get()

            # Checkpoint the current block (if it is not the last)
            if self._checkpoints_dir is not None and last_block:
                # Only save the ones that are in the overlap or are connected to the overlapping tilespecs
                relevant_sec_idxs = set(list(range(block_hi - (self._block_size - self._block_step) - 1, block_hi)))
