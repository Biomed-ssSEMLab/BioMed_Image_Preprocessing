...
def ransac(sample_matches, test_matches, target_model_type, iterations, epsilon, min_inlier_ratio, min_num_inlier, det_delta=0.55, max_stretch=None, max_rot_deg=None, tri_angles_comparator=None, max_distance=None):
    ...
    #print("proposed_model", proposed_model)
    #print("target_model_type:", target_model_type)
    #print("array of proposed_model", proposed_model.m)
    '''
    if (proposed_model.m).shape != (3,3):
        raise RuntimeError("Proposed model type wrong")
    '''
    #max_rot_deg = 180
    ...
    #print("proposed_model.MIN_MATCHES_NUM is: ", proposed_model.MIN_MATCHES_NUM)
    #print("sample_matches[0].shape[0] is: ", sample_matches[0].shape[0])
    ...
    if proposed_model.MIN_MATCHES_NUM > sample_matches[0].shape[0]:
        ...
        print("proposed_model.MIN_MATCHES_NUM is small")
    ...
    max_combinations = int(comb(len(sample_matches[0]), proposed_model.MIN_MATCHES_NUM))  # comb方法用于从N个数中挑选K个数的选项总共多少种
    ...
    for min_matches_idxs in choices:
        #print("proposed_model:", (proposed_model.m).shape)
        '''
        if (proposed_model.m).shape != (3,3):
            raise RuntimeError("Proposed model type wrong, should be of size 3x3")
        '''
        ...
        if proposed_model.MIN_MATCHES_NUM == 3:
            ...
            if tri_angles_comparator is not None:
                #print("tri_angles_comparator")
                if tri_angles_comparator(proposed_model) is False:
                    #print("tri_angles")
                    continue
        '''
        if (proposed_model.m).shape != (3,3):
            raise RuntimeError("Proposed model5 type wrong, should be of size 3x3")
        '''
        #print("proposed model5:", proposed_model.m)
        ...
        if max_distance is not None:
            ...
            '''
            if (proposed_model.m).shape != (3,3):
                #print("proposed_model6:", proposed_model.m)
                raise RuntimeError("Proposed model6 type wrong, should be of size 3x3")
            '''
        ...
        print("proposed model before score:", proposed_model.m)
        #time.sleep(5)
        '''
        if (proposed_model.m).shape != (3,3):
            #print("proposed_model copy before score to best model:", (best_model.m))
            raise RuntimeError("Proposed_model copy to Best model before score type wrong, should be of size 3x3")
        '''
        ...
        '''
        if (proposed_model.m).shape != (3,3):
            #print("proposed_model copy to best model:", (best_model.m))
            raise RuntimeError("Proposed_model copy to Best model type wrong, should be of size 3x3")
        '''
    ...
    '''
    '''
    if (best_model.m).shape != (3,3):
        print("best model:", np.array(best_model).shape)
        raise RuntimeError("Best model type wrong, should be of size 3x3")
    '''
...
def filter_matches(sample_matches, test_matches, target_model_type, iterations, epsilon, min_inlier_ratio, min_num_inlier, max_trust, det_delta=0.35, max_stretch=None, max_rot_deg=None, robust_filter=True, tri_angles_comparator=None, max_distance=None):
    ...
    meandists = -1
    print("tartget model type in ransac match-filter:", target_model_type)
    # Apply RANSAC
    ...
    if model is None:
        print("In ransac.py: the model after RANSAC is none")
    ...
    if filtered_matches is None:
        ...
        inliers_mask[inliers_mask == True] = filtered_inliers_mask # 靳海群修改，clear everrything that's not set by filtered_inliers_mask
