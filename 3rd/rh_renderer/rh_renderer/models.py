...
class AbstractModel(object):
    ...
    def score(self, X, y, epsilon=100.0, min_inlier_ratio=0.01, min_num_inlier=7):
        ...
        if good_dists_num < min_num_inlier or accepted_ratio < min_inlier_ratio:
            print("good_dists_num < min_num_inlier: ", good_dists_num,  min_num_inlier)
            print("accepted_ratio < min_inlier_ratio: ", accepted_ratio, min_inlier_ratio)
...
class RigidModel(AbstractAffineModel):
    ...
    def get_matrix(self):
        return np.array([[self.cos_val, -self.sin_val, self.delta[0]],
...
class SimilarityModel(AbstractAffineModel):
    ...
    def get_matrix(self):
        return np.array([np.array([self.scos_val, -self.ssin_val, self.delta[0]]),
...
class AffineModel(AbstractAffineModel):
    ...
    def __init__(self, m=np.eye(3)):
        ...
        self.set(m)
        print("m after set:", m)
        if np.array(m).shape!=(3,3):
            raise RuntimeError("the matrix should be of size 3x3")
    ...
    def apply_inv(self, p):
        ...
        pts = np.atleast_2d(p)
        return np.dot(self.m_inv[:2,:2], pts.T).T + np.asarray(self.m_inv.T[2][:2]).reshape((1, 2))
    ...
    def set_from_modelspec(self, s):
        ...
        self.m = np.vstack(np.array([float(d) for d in splitted[0::2]]),
    ...
    def fit(self, X, y):
        ...
        X and y are assumed to be arrays of 2D points of the same shape.
        """
        assert(X.shape[0] >= 2) # the minimal number of of matches for a 2d affine transformation
        ...
        self.m = np.array([[m00, m01, qc[0] - m00 * pc[0] - m01 * pc[1]],
...
class PointsTransformModel(AbstractModel):
    ...
    def set_from_modelspec(self, s):
        ...
        src = np.array([np.array(points_data[0::5], dtype=np.float32),
                        np.array(points_data[1::5], dtype=np.float32)]).T
        dest = np.array([np.array(points_data[2::5], dtype=np.float32),
                         np.array(points_data[3::5], dtype=np.float32)]).T
