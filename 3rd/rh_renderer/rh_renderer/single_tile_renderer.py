...
class SingleTileRendererBase(object):
    ...
    def render(self):
        ...
        if self.non_affine_transform is None:
            ...
            img_shape = img.shape
            img = cv2.warpAffine(img, adjusted_transform, self.shape, flags=cv2.INTER_AREA)
            if self.compute_mask:
                mask_img = np.ones(img_shape)
                mask = cv2.warpAffine(mask_img, adjusted_transform, self.shape, flags=cv2.INTER_AREA)
                mask[mask > 0] = 1
            ...
            if self.compute_distances:
                ...
                weights_img = np.minimum(np.minimum(grid[0], self.height - 1 - grid[0]),
                                         np.minimum(grid[1], self.width - 1 - grid[1])).astype(np.float32) + 0.5
                weights = cv2.warpAffine(weights_img, adjusted_transform, self.shape, flags=cv2.INTER_AREA)
        else:
            st_time = time.time()
            ...
            img_shape = img.shape
            img = cv2.remap(img, map_x, map_y, cv2.INTER_CUBIC).T
            ...
            if self.compute_mask:
                mask_img = np.ones(img_shape)
                mask = cv2.remap(mask_img, map_x, map_y, cv2.INTER_CUBIC).T
                mask[mask > 0] = 1
                mask = mask.astype(np.uint8)
            if self.compute_distances:
                # The initial weights for each pixel is the minimum from the image boundary
                ...
                weights_img = np.minimum(np.minimum(grid[0], self.height - 1 - grid[0]),
                                         np.minimum(grid[1], self.width - 1 - grid[1])).astype(np.float32) + 0.5
                weights = cv2.remap(weights_img, map_x, map_y, cv2.INTER_CUBIC).T
                weights[weights < 0] = 0
    ...
    def fetch_mask(self):
        ...
        img, mask, weights = self.render()
        return mask, (self.bbox[0], self.bbox[2])
    ...
    def crop(self, from_x, from_y, to_x, to_y):
        ...
        img, mask, _ = self.render()
        # Check with the actual image bounding box (may be different because of the non-affine transformation)
        actual_bbox = [self.start_point[0], self.start_point[0] + img.shape[1], self.start_point[1], self.start_point[1] + img.shape[0]]
        ...
        cropped_img = img[int(overlapping_area[2] - actual_bbox[2]): int(overlapping_area[3] - actual_bbox[2] + 1),
                          int(overlapping_area[0] - actual_bbox[0]): int(overlapping_area[1] - actual_bbox[0] + 1)]
        if self.compute_mask:
            cropped_mask = mask[int(overlapping_area[2] - actual_bbox[2]): int(overlapping_area[3] - actual_bbox[2] + 1),
                                int(overlapping_area[0] - actual_bbox[0]): int(overlapping_area[1] - actual_bbox[0] + 1)]
    ...
    def crop_with_distances(self, from_x, from_y, to_x, to_y):
        ...
        cropped_img = img[int(overlapping_area[2] - actual_bbox[2]): int(overlapping_area[3] - actual_bbox[2] + 1),
                          int(overlapping_area[0] - actual_bbox[0]): int(overlapping_area[1] - actual_bbox[0] + 1)]
        if self.compute_distances:
            cropped_distances = weights[int(overlapping_area[2] - actual_bbox[2]): int(overlapping_area[3] - actual_bbox[2] + 1),
                                        int(overlapping_area[0] - actual_bbox[0]): int(overlapping_area[1] - actual_bbox[0] + 1)]
...
class SingleTileDynamicRendererBase(SingleTileRendererBase):
    def __init__(self, width, height, 
                 ...
                 compute_mask=False, 
                 compute_distances=True):
        super(SingleTileDynamicRendererBase, self).__init__(width, height, bbox, transformation_models, compute_mask, compute_distances)
...
class SingleTileStaticRenderer(SingleTileRendererBase):
    ...
    def __init__(self, img_path, width, height, 
                 ...
                 compute_distances=True,
                 hist_adjuster=None):
        super(SingleTileStaticRenderer, self).__init__(width, height, bbox, transformation_models, compute_mask, compute_distances)
...
class SingleTileRenderer(SingleTileDynamicRendererBase):
    ...
    def __init__(self, img_path, width, height, 
                 ...
                 compute_distances=True,
                 hist_adjuster=None):
        super(SingleTileRenderer, self).__init__(width, height, bbox, transformation_models, compute_mask, compute_distances)
        self.img_path = img_path
        self.hist_adjuster = hist_adjuster
...
class AlphaTileRenderer(SingleTileDynamicRendererBase):
    ...
    def __init__(self, other_renderer):
        ...
        '''
        super(AlphaTileRenderer, self).__init__(other_renderer.width, other_renderer.height, None, [], False, False)
        pre, post = [AffineModel(np.vstack([transform, [0, 0, 1]])
