...
def read_bboxes(all_files):
    all_bboxes = []
    for file in all_files:
        if not os.path.isfile(file):
            raise Exception("Error! {} is not exist!".format(file))

        with open(file, "r") as f:
            json_dict = json.load(f)
            for i in range(len(json_dict)):
                all_bboxes.append(json_dict[i]["bbox"])
...
def parse_workflow_folder(cur_fs, workflow_folder):
    ...
    result = {}
    all_sec_folders = sorted(cur_fs.glob("/*_S*R1/"))


    # all_sec_folders = sorted(glob.glob('/media/liuxz/3EA0B4CEA0B48E41/output/001_S1R1'))
    for sec_folder_glob in all_sec_folders:
        ...
        if m is not None:
            ...
            elif cur_fs.exists(sec_folder + 'full_image_coordinates_5.txt'):
                image_coordinates_files = sec_folder + 'full_image_coordinates_5.txt'
            else:
...
def read_layer_from_filename(fname):
    if os.path.basename(fname)[:4].isdigit():
        num = int(os.path.basename(fname)[:4])
    else:
        base_fname = os.path.basename(fname)  # fname should be like: S1M1.json or S1M1-2.json
        if base_fname.find("S") == -1 or base_fname.find("M") == -1 or base_fname.find("S") > base_fname.find("M"):
            raise Exception("Error! fname {} should be like: S1M1.json or S1M1-2.json".format(fname))
        tmp_str = base_fname[base_fname.find("S") + 1:]
        tmp_str = tmp_str[:tmp_str.find("M")]
        if not tmp_str.isdigit():
            raise Exception("Error! fname {} should be like: S1M1.json or S1M1-2.json".format(fname))
        num = int(tmp_str)
    return num

def get_all_layer_idx(all_json_files):
    all_json_files = sorted(all_json_files)
    all_layer_idx = []
    for json_file in all_json_files:
        idx = read_layer_from_filename(json_file)
        all_layer_idx.append(idx)
    return all_layer_idx

def filter_files_layers(all_json_files, from_layer, to_layer):
    all_json_files = sorted(all_json_files)
    filtered_files = []
    filtered_layers_idx = []
    for json_file in all_json_files:
        idx = read_layer_from_filename(json_file)
        if idx in range(from_layer, to_layer + 1):
            filtered_files.append(json_file)
            filtered_layers_idx.append(idx)
    return filtered_files, filtered_layers_idx
